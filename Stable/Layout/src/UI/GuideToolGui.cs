...
namespace Layout.UI
{
    ...
    public class GuideToolGui : GuiDialog
    {
        ...
        // The shape catalog — the four entries of the primitives+modifiers model, in the
        // order players think of them. Codes map to (GuideShapeType, ShapeConstraint).
        private static readonly string[] ShapeCodes = { "arch", "halfcircle", "circle", "ellipse" };
        private static readonly string[] ShapeNames = { "Arch", "Half-circle", "Circle", "Ellipse" };
        ...
        private void SetupDialog()
        {
            ...
            // ---- Shape row: the initial-shape picker — ALWAYS present (playtest fix #1) ----
            AddTileRow(c, mainLabelFont, mainTileFont, ref y, labelW, controlW, pad, rowH, rowGap,
                "Shape", ShapeCodes, ShapeNames, CurrentShapeIndex(), OnShapeTile, "shape", deleteMode);
        ...
        // Lights each row's current selection immediately after composition.
        ...
        {
            if (_tool.Shape == GuideShapeType.Ellipse)
                return _tool.Constraint == ShapeConstraint.Circle ? 2 : 3;
            return _tool.Constraint == ShapeConstraint.SemiCircle ? 1 : 0;
        }

        internal static string ShapeDisplayName(GuideShapeType shape, ShapeConstraint constraint)
        {
            if (shape == GuideShapeType.Ellipse)
                return constraint == ShapeConstraint.Circle ? "Circle" : "Ellipse";
            return constraint == ShapeConstraint.SemiCircle ? "Half-circle" : "Arch";
        }

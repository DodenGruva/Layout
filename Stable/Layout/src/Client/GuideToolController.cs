...
namespace Layout.Client
{
    ...
    public sealed class GuideToolController : IDisposable
    {
        ...
        public void OnPrimaryClick(BlockSelection blockSel)
        {
            ...
            if (hit.Found)
            {
                ...
                if (hit.PointIndex >= 0)
                {
                    StartGrab(hit.GuideId, hit.PointIndex);
                         ...
                         && bodyG.ShapeType == GuideShapeType.Ellipse)
                {
                    // DECISION (Session 8, flagged for review): a body grab on the ellipse family grabs the
                    // NEAREST HANDLE — pulling the ring stretches it, which is the natural feel; an
                    // arbitrary interpolation point has no meaning on a parametric ellipse, so there is
                    // nothing to insert. (Grabbing the ring of a CIRCLE therefore resizes it via a diameter
                    // anchor, or breaks it to an ellipse when the minor handle is what you caught.)
            ...
            // 5. Nothing of ours under the crosshair → this is a draft-anchor click.
            _draft.ClearSelection();
            HandleCreateClick(blockSel);
        }

        /// <summary>Right-click: cancel grab / cancel draft / toggle a point's lock, by context.</summary>
        public void OnSecondaryClick(BlockSelection blockSel)
        {
            ...
            else if (g.ShapeType == GuideShapeType.Ellipse)
            {
                // DECISION (Session 8, flagged for review): lock-in-place has no meaning on a parametric
                // ellipse (no arbitrary points exist to be born there), so a body right-click toggles the
                // NEAREST HANDLE's lock instead — the same "lock what I'm pointing at" intent, mapped to
                // the points this shape actually has.

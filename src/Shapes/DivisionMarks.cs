    ...
    public static class DivisionMarks
    {
        ...
        public static void Apply(List<VoxelPosition> cells, List<Vec3d> curve, int divisions, int scale)
        {
            ...
            for (int k = first; k <= last; k++)
            {
                ...
                // Paired claim: when a boundary lands between two voxels, mark BOTH so the parts read even —
                // the same midpoint treatment as the arch apex (human-requested).
                ShapeGeometry.ClaimMarkerPaired(cells, scale, p, VoxelRenderType.Division);
            }
        }

...
- **Versioning rule (standing, human-set):** EVERY revision bumps `modinfo.json` and ships as a NEW
  ...
  never overwrite an older release zip. Versions increment monotonically per revision. **Current: v0.3.40.**
...
- **`ARCHITECTURE.md`** (in `dev/`, v3.11) — the authoritative plan. Its **Settled Decisions Register** lists
...
- **`SESSION_9.md` … `SESSION_24.md`** — standalone per-session records; SESSION_12 covers **v0.1.28–v0.1.45
  ...
  **v0.2.24–v0.2.28 Tapered Cylinder arc** (the 4-click frustum at protocol 7; the scan-guard and
  ...
  SESSION_24 the **v0.3.35–v0.3.40 materialization-completion, shell-transition, and HUD-dimension arc**.

## ✅ Docs verified & consolidated to v0.3.40 (2026-07-23)
The authoritative prose docs are consistent with **v0.3.40, DataVersion 12, protocol 16, 77 source files,
15 shape types / 21 tiles**. `ARCHITECTURE.md` is **v3.11**; `SESSION_24.md` is the latest record
...
## Current priorities (detail in TODO.md; mesh plan in SESSION_14.md §6–§7, latest record in SESSION_24.md)
...
2. **Field-test v0.3.40.** Stress small and immense Shell materialization, Wireframe→Shell Edit transitions,
   the full-growth hold and clean-shell swap, post-placement effects, repeated sculpt resize/release,
   polygonal-prism startup, and intrinsic HUD dimensions.
3. **Performance follow-up only from measurements.** Immense placement/sculpt geometry now uses one
   below-normal worker plus bounded claim ticks; the client streams organic selected-scale batches and retires
   old meshes across frames. Mesh Stage B/C remains optional if settled in-range drawing becomes the bottleneck.

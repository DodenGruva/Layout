...
(custom deflating 4-state model) with **finite, powder-refillable chalk durability** (F5).
...
- **Versioning rule (standing, human-set):** EVERY revision bumps `modinfo.json` and ships as a NEW
  ...
  never overwrite an older release zip. Versions increment monotonically per revision. **Current: v0.2.9.**
- **Git:** `main` is the mainline, pushed to **github.com/DodenGruva/Layout** through v0.2.9 (the
  `ClientOnlyFallback` branch was merged via PR #1). Commit/push ONLY when the human instructs.
...
- **`ARCHITECTURE.md`** (in `dev/`, v3.2) — the authoritative plan. Its **Settled Decisions Register** lists
...
- **`SESSION_9.md` … `SESSION_15.md`** — standalone per-session records; SESSION_12 covers **v0.1.28–v0.1.45
  ClientOnlyFallback**, SESSION_13 the **v0.1.46–v0.1.52 optimization and interaction pass**, SESSION_14 the
  **v0.1.53 hollow-shell + mesh optimization handoff** (read before continuing performance work), and
  SESSION_15 the **v0.2.0–v0.2.9 Chalking Kit arc** (reskin, ground storage, F5 durability, fill-state
  models, effects).

## ✅ Docs verified & consolidated to v0.2.9 (2026-07-18)
The authoritative prose docs are consistent with **v0.2.9, DataVersion 8, protocol 4, 68 source files,
12 shape types / 18 tiles**. `ARCHITECTURE.md` is **v3.2**; `SESSION_15.md` is the detailed F5 record;
...
## Current priorities (detail in TODO.md; mesh handoff in SESSION_14.md, F5 record in SESSION_15.md)
F4 (client-only/private guides) and F5 (Chalking Kit durability) are both feature-complete and playtested
through **v0.2.9** on `main`. Normal public multiplayer behavior remains in place.
...
2. **Finish the B-S9-1 interaction regression.** Lock placement no longer shifts the guide (v0.1.51/52), but
   repeated lock/drag/revert/unlock cycles need more playtesting before the bug is closed.
3. **The final F4/public multiplayer regression pass** (vanilla-server fallback, policy denial, mixed
   public/private, push, reconnect, public-only regression) — still owed before any release-grade stamp.
4. **F5 held item:** cursor-stack inventory refill — the human may instead require ground refills; decision
   pending playtest. Effects tuning (puff size, snap volume) by feel.

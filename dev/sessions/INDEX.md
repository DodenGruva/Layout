...
|---|---|---|---|
| 41 | 0.4.55–0.4.59 | — | **Independent review hardening, followed by adversarial review of its own fixes.** Whole-guide Move, Rotate, Transform, Copy, and projection paths now share coordinate/plane validation, closing a reachable 2^27-position server-hang gap. Combined rotation, mirror, and translation are one validated transaction with one rollback and one undo entry. v0.4.56 moved malformed-input validation ahead of the no-op return. v0.4.57 carries cancellation through every expensive immense-volume stage. v0.4.58 replaces count-only claim revision with a fail-closed structural snapshot; v0.4.59 bounds it to the exact guide footprint so distant claims cannot restart or inflate validation while exact per-block access remains unchanged. The focused harness passes 29/29 groups; the final 10,000-claim snapshot measured 0.10–0.14 ms / about 820 bytes with one relevant claim. **v0.4.55 and v0.4.57 were not playtested while implemented because the human explicitly said they would be away; no dedicated-server result for the v0.4.58–v0.4.59 claim changes has been reported.** |
